...
# Два друга A и B постоянно играют в коллекционную карточную игру (ККИ), поэтому у каждого игрока скопилась довольно
...
#   - Если среди лежащих на столе карт игрока A есть такая же карта, как и среди лежащих карт игрока B — каждый игрок
#   убирает данную карту со стола;
...
# Даны начальные состояния коллекций игроков, а также Q изменений их коллекций. После каждого изменения необходимо
# вычислить разнообразие коллекций друзей.
...
# Далее на каждой из следующих Q строк описано изменение коллекции: через пробел заданы type k player k card k
# (type k = ± 1; player k ∈ (A, B); 1 ≤ card k ≤ 10^9) — тип k-го изменения, имя игрока и
#  значение карты:
...
# Примечания
# В первом примере после первого изменения коллекции следующие:
# 1, 2, 3 и 1, 2, 3, 4, 5, разнообразие - 2, т.к. игроки уберут общие карты 1, 2, 3, оставив 4, 5.
# Далее поочередно первому игроку добавляются 4, 5, сокращая различие между коллекциями игроков на 1 в обоих случаях.
# Четвёртое и пятое изменение добавляет первому игроку карты 6, 7, которых нет у игрока B, поэтому разнообразие
# увеличивается на 1 в обоих случаях.
...
# Во втором примере на первом шаге коллекции 1000, 2000, 1001, 100000; 1001, 2001, 1000 - на столе останутся 2000,
# 100000 и 2001.
# На втором шаге разнообразность понижается на 1, т.к. 2001 была только в коллекции второго игрока.
# На третьем шаге второму игроку добавляется карта 2000, которая есть у первого игрока, поэтому разнообразие
# уменьшается до 1.
