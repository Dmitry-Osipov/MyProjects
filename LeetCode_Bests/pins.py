...
the function getPINs (get_pins in python, GetPINs in C#). But please note that all PINs, the observed one and also the 
results, must be strings, because of potentially leading '0's. We already prepared some test cases for you.
Detective, we are counting on you!
"""


def get_pins_hard(observed: str) -> list:
    if not observed:
        return ['']
    ...
    residue_pins = get_pins_hard(observed[1:])
    digit = observed[0]

    return [digit + pin for digit in all_ways[digit] for pin in residue_pins]


def get_pins_easy(observed: str) -> list:
    all_ways = {
        '1': ['1', '2', '4'],
        '2': ['2', '1', '3', '5'],
        '3': ['3', '2', '6'],
    ...
    digits = [all_ways[digit] for digit in observed]
    all_combo = product(*digits)

    return [''.join(combo) for combo in all_combo]


some_pin = '1357'

result_hard = get_pins_hard(some_pin)
print(f'hard way: {len(result_hard)}')

result_easy = get_pins_easy(some_pin)
print(f'easy way: {len(result_easy)}')
